...
import com.zixu.paysapi.mvc.util.GetHttpSessionConfigurator;
...
public class WebSocketTest {
    ...
    public void onOpen(Session session) throws IOException{
       ...
       /* String req = session.getQueryString();
        if(req == null || req.indexOf("uid=") == -1) {
        	session.close();
        	return;
        }*/
    ...
    public void onMessage(String res, Session session) {
    	...
    	if(jsonObject.get("qrcode") != null) {
    		 System.out.println("来自客户端的消息:" + res);
    		 for(WebSocketTest item: webSocketSet){
	            try {
	                item.sendMessage("service:"+res);
	            } catch (IOException e) {
	                e.printStackTrace();
	                continue;
	            }
